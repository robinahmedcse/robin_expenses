...
[![Build Status {{asset('public/backEnds/')}}/images/](https://travis-ci.org/fengyuanchen/cropper.svg)](https://travis-ci.org/fengyuanchen/cropper)
...
**Note:** Don't trust this all the time as some JPG {{asset('public/backEnds/')}}/images/ have incorrect (not standard) Orientation values.
...
- **onlyColorChanged** (optional):
  - Type: `Boolean`
  - If only change the color, not the size, then the cropper only need to change the srcs of all related {{asset('public/backEnds/')}}/images/, not need to rebuild the cropper. This can be used for applying filters.
